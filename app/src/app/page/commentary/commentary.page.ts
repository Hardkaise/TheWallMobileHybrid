...
export class CommentaryPage implements OnInit {
  ...
  login(form: any){
    ...
    const data = {  ownerId: this.apiService.currentUserId, imageId: this.id, text: form.form.value.addcomment}
    console.log(data);
    let textcomment = "";
    this.apiService.create('image-commentaries', data).then(payload => {
      console.log(payload);
    })
  }
  ngOnInit() {
